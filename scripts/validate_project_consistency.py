...
REQUIRED_FILES = [
    ...
    "retail_ops/data/store_period_panel_source_notes.md",
    "retail_ops/sql/03_store_period_panel_coverage.sql",
    "retail_ops/sql/04_repeated_window_panel_summary.sql",
    "retail_ops/outputs/store_period_panel_coverage_output.csv",
    "retail_ops/outputs/repeated_window_panel_summary_output.csv",
    "retail_ops/scripts/validate_store_period_panel.py",
    "retail_ops/scripts/validate_repeated_window_panel_summary.py",
    "retail_ops/outputs/store_period_panel_validation_result.txt",
    "retail_ops/outputs/repeated_window_panel_summary_validation_result.txt",
    ...
    "eval/eval_retail_demo2_endpoint_behavior.py",
]
...
def normalize_markdown_table_text(text: str) -> str:
    ...
    for line in text.splitlines():
        ...
        for cell in cells:
            ...
            if compact and "-" in compact and set(compact) <= {"-", ":"}:
...
def check_markdown_tables(failures: list[str]) -> None:
    for rel, required_patterns in REQUIRED_MARKDOWN_TABLE_PATTERNS.items():
        ...
        normalized_text = normalize_markdown_table_text(read_text(path))

        for pattern in required_patterns:
            normalized_pattern = normalize_markdown_table_text(pattern)

            if normalized_pattern not in normalized_text:
